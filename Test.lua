...
function M.getHRP()
    local char = (Player and Player.Character) or (LP and LP.Character) or (game.Players.LocalPlayer and game.Players.LocalPlayer.Character)
    if not char then return nil end
    return char:FindFirstChild("HumanoidRootPart")
end

function M.getCursedShipEntryPos()
    return M.CursedShipOutsideDoor or Vector3.new(902.059143, 124.752518, 33071.8125)
end
...
RunService.Stepped:Connect(function()
    pcall(function()
        -- Verifica se o jogador quer uma velocidade acima do padrão (16)
        if S.SetWalkSpeed and S.SetWalkSpeed > 16 then
            local localPlayer = game.Players.LocalPlayer
            if localPlayer.Character and localPlayer.Character:FindFirstChild("Humanoid") then
                local humanoid = localPlayer.Character.Humanoid
                if humanoid.WalkSpeed ~= S.SetWalkSpeed then
...
function M.isFruitTool(tool)
    ...
    return tip == "demon fruit" or tip == "blox fruit" or tip:find("eat to gain", 1, true) ~= nil
end
...
function M.getDoughGroundPosition(pos)
    ...
    if not _G.NexusHubGroundRayParams then
        _G.NexusHubGroundRayParams = RaycastParams.new()
        _G.NexusHubGroundRayParams.FilterType = Enum.RaycastFilterType.Exclude
    end
    local params = _G.NexusHubGroundRayParams
    local ignore = {}
    local enemies = Workspace:FindFirstChild("Enemies")
...
-- GLOBAL
local version = NexusHubVersion or "2.1.0"
...
local function AutoHaki()
    ...
    task.spawn(function()
        repeat task.wait() until (player.Character and player.Character:FindFirstChild("HumanoidRootPart")) or not _G.NexusHubRunning
        while _G.NexusHubLoaded and _G.NexusHubRunning do
            task.wait(2)
            if S.autoBuso then
                pcall(M.enableBuso)
            end
